...
export default function AdminSubmissionsCampaignsPage() {
    ...
    try {
      const [submissionsResult, campaignsResult] = await Promise.all([
        supabase
          ...
          .select(
            `
            id, task_id, worker_id, status, reward_amount, created_at,
            campaign_tasks!inner ( id, title, campaign_id )
          `
          )
          .order("created_at", { ascending: false }),
        supabase.from("campaigns").select("id, title"),
      ...
      const mapped: Submission[] = (submissionsResult.data || []).map((s: any) => {
        const task = s.campaign_tasks;
        const campaign = task ? campaignMap.get(task.campaign_id) : undefined;
        return {
          ...s,
          ...
          task: task
            ? { id: task.id, title: task.title, campaign_id: task.campaign_id }
            : undefined,
          campaign: campaign
            ? { id: campaign.id, title: campaign.title }
            : undefined,
  ...
  // Realtime – refresh cards + trigger notification helper
  useEffect(() => {
    const channel = supabase
      .channel("admin-submissions-overview")
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "task_submissions" },
        () => {
          loadData(true);
          // In-app notification is handled on the detail page & via push
        }
      )
      .subscribe();
  ...
  return (
    <div className="space-y-7">
      ...
      {campaignCards.length === 0 ? (
        <div className="rounded-2xl border border-slate-200 bg-white px-6 py-20 text-center shadow-sm">
          <Megaphone className="mx-auto h-12 w-12 text-slate-300" />
          <h3 className="mt-4 font-semibold text-slate-700">No campaigns found</h3>
          <p className="mx-auto mt-2 max-w-md text-sm text-slate-500">
            Campaigns with task submissions will appear here.
          </p>
        </div>
        ...
        <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
          {campaignCards.map((card) => (
            <button
              key={card.id}
              type="button"
              onClick={() => router.push(`/admin/dashboard/submissions/${card.id}`)}
              className="group rounded-2xl border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:border-[#0b3939]/40 hover:shadow-md"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
              ...
              <div className="mt-5 grid grid-cols-3 gap-2 border-t border-slate-100 pt-4">
                <div>
                  <p className="text-xs text-slate-400">Pending</p>
                  <p className="mt-0.5 text-sm font-bold text-amber-600">{card.pending}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-400">Approved</p>
                  <p className="mt-0.5 text-sm font-bold text-emerald-600">{card.approved}</p>
                </div>
                <div>
                  <p className="text-xs text-slate-400">Rejected</p>
                  <p className="mt-0.5 text-sm font-bold text-red-600">{card.rejected}</p>
                </div>
              </div>
