...
const campaignId = params.id as string;
  ...
  const [campaignTitle, setCampaignTitle] = useState("Campaign submissions");
    ...
    async (isRefreshing = false) => {
      ...
      try {
        ...
        if (campaignResult.data) {
          setCampaignTitle(
            campaignResult.data.title || "Campaign submissions"
          );
        }
        ...
        const mapped: Submission[] = submissionsData
          .filter((s) => {
            const task = taskMap.get(s.task_id);
            return task?.campaign_id === campaignId;
          })
          .map((submission) => {
            ...
            return {
              ...
              campaign: {
                id: campaignId,
                title: campaignResult.data?.title || null,
              },
              worker: worker
                ? {
                    id: worker.id,
  ...
  useEffect(() => {
    const channel = supabase
      .channel(`admin-submissions-${campaignId}`)
      .on(
        "postgres_changes",
        {
          event: "INSERT",
        ...
        async (payload) => {
          ...
          // Local notification when the tab is open
          if (typeof window !== "undefined" && Notification.permission === "granted") {
            const newSub = payload.new as any;
            new Notification("New Task Submission", {
              body: `A worker just submitted proof. Reward: ₦${Number(
                newSub.reward_amount || 0
              ).toLocaleString()}`,
              icon: "/icon-192.png",
              tag: `submission-${newSub.id}`,
  ...
  const enablePushNotifications = async () => {
    ...
    try {
      ...
      if (!subscription) {
        ...
        if (!vapidPublicKey) {
          throw new Error("VAPID public key is missing (NEXT_PUBLIC_VAPID_PUBLIC_KEY).");
        }
      ...
      setPushEnabled(true);
      setSuccessMessage(
        "Push notifications enabled! You will be notified even when the phone is locked."
      );
    } catch (err) {
      console.error(err);
      setError(
  ...
  }, [submissions, searchQuery, statusFilter, taskFilter]);
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button
            ...
            className={`inline-flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-semibold shadow-sm disabled:opacity-60 ${
              pushEnabled
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="relative inline-flex items-center gap-1">
                            ...
                            <button
                              ...
                              className="flex h-9 w-9 items-center justify-center rounded-lg text-blue-500 hover:bg-slate-100"
                            >
  ...
  disabled={
    actionLoading ||
    selectedSubmission.status !== "pending" ||          // ← disables if already reviewed
